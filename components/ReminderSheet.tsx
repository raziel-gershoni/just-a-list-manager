...
export default function ReminderSheet({
  ...
  return (
    ...
    >
      <div
        className="bg-tg-bg w-full max-w-lg rounded-t-2xl p-6 max-h-[80vh] overflow-y-auto"
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-lg font-semibold text-tg-text">{t("title")}</h2>
        ...
        {/* Existing reminder */}
        {existingReminder && (
          <div className="mb-4 p-3 bg-tg-secondary-bg rounded-xl">
            <p className="text-sm text-tg-text">
              {t("active", { time: formatExistingTime(existingReminder.remind_at) })}
            </p>
            <button
              onClick={handleCancel}
              className="mt-2 px-4 py-1.5 rounded-full text-sm font-medium bg-red-500/10 text-red-500"
            >
              {t("cancel")}
            </button>
          </div>
        ...
        <div className="mb-5">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-tg-secondary-bg text-tg-text rounded-lg px-3 py-2 text-sm"
            />
            <button
              onClick={handleCustomSet}
              disabled={!customDateTime}
              className="px-4 py-2 rounded-lg bg-tg-button text-tg-button-text text-sm font-medium disabled:opacity-50"
            >
              {t("set")}
            </button>
          </div>
