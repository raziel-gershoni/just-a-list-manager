 ...
 * Uses SHA256(bot_token) as secret (different from Mini App initData).
 * Returns the user if valid, null if invalid.
 *
...
export function validateLoginWidget(
  ...
  try {
    ...
    if (!hash) {
      console.warn("[TelegramAuth] No hash in login widget data");
      return null;
    }
    ...
    if (isNaN(authDate) || now - authDate > maxAge) {
      console.warn("[TelegramAuth] Login widget data expired");
      return null;
    }
    ...
    // Secret key: SHA256(bot_token) — different from Mini App!
    ...
    if (
      ...
      console.warn("[TelegramAuth] Login widget hash mismatch");
      return null;
    }
    ...
    console.error("[TelegramAuth] Login widget validation error:", error);
    return null;
  }
}
