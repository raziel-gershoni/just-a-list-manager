...
export function useListData(listId: string, jwtRef: React.RefObject<string | null>) {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      if (listsRes.ok) {
        ...
        const currentList = allLists.find((l: { id: string; name: string; is_shared?: boolean }) => l.id === listId);
        if (currentList) {
  ...
  return { listName, setListName, items, setItems, loading, error, isShared, setIsShared, fetchItems, refreshItems };
}
